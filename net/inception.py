...
class Inception3(nn.Module):
    ...
    def features_grad_multi_layers(self, x: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        # N x 192 x 35 x 35
        x_5b = self.Mixed_5b(x)
        x_5b.retain_grad()
        # N x 256 x 35 x 35
        x_5c = self.Mixed_5c(x_5b)
        # N x 288 x 35 x 35
        x_5d = self.Mixed_5d(x_5c)
        x_5d.retain_grad()

        # N x 288 x 35 x 35
        x_6a = self.Mixed_6a(x_5d)
        # N x 768 x 17 x 17
        x_6b = self.Mixed_6b(x_6a)
        x_6b.retain_grad()
        # N x 768 x 17 x 17
        x_6c = self.Mixed_6c(x_6b)
        # N x 768 x 17 x 17
        x_6d = self.Mixed_6d(x_6c)
        # N x 768 x 17 x 17
        x_6e = self.Mixed_6e(x_6d)
        x_6e.retain_grad()
        ...
        if self.AuxLogits is not None:
            if self.training:
                aux = self.AuxLogits(x_6e)
        # N x 768 x 17 x 17
        z = self.Mixed_7a(x_6e)
        ...
        z = self.fc(z)
        # N x 1000 (num_classes)
        return z, x_5b, x_5d, x_6b, x_6e

    def multi_layer_features(self, x: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        # N x 3 x 299 x 299
        x = self.Conv2d_1a_3x3(x)
        # N x 32 x 149 x 149
        x = self.Conv2d_2a_3x3(x)
